...
def recommend(movie):
    ...
    for i in distances[1:51]:  # Fetching 20 posters
...
movie_list = movies['title'].values  # Define movie_list here

# Setting page header for the Movie Recommender System
        ...
        for j in range(cols):
            ...
            if index < len(recommended_movie_names):
                with cols_list[j]:
                    st.markdown(
                        f"""
                        <div style='color: white;'>{recommended_movie_names[index]}</div>
                        """,
                        unsafe_allow_html=True
                    )
